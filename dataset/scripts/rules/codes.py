...
SRC = {
    ...
    "MCP_TS_SDK": "https://github.com/modelcontextprotocol/typescript-sdk/blob/main/packages/core-internal/src/wire/rev2025-11-25/buildSchemas.ts",
...
CODES = {
    ...
    # These five are emitted by the rule modules but were never registered
    # here, so lookup() fell through to ("?", "unknown", ..., "") and any
    # consumer that hit one got an unresolved axis and an empty source quote.
    # No row in the published corpus carries any of them (axis A is 0/617, and
    # B2-root-not-object / B4-untyped-open-object have 0 hits -- see STATS.txt),
    # so registering them moves no verdict, count, or rate. The `z.*` quotes
    # come from the TypeScript SDK's wire schema, cited as [A-src2] in
    # rules/mcp_openai.py, hence MCP_TS_SDK rather than the JSON spec URL.
    "A0-tool-not-object":           ("A", "hard-reject", "MCP_TS_SDK",
                                     "ListToolsResultSchema = PaginatedResultSchema.extend({ tools: z.array(ToolSchema) })"),
    "A4-properties-not-object":     ("A", "hard-reject", "MCP_TS_SDK",
                                     "properties: z.record(...).optional()"),
    "A6-outputSchema-not-object":   ("A", "hard-reject", "MCP_SPEC",
                                     'outputSchema carries the same {"const":"object"} type constraint'),
    ...
    # emitted spellings (see the A0/A4 note above -- 0 hits in the corpus)
    "B2-root-not-object":           ("B", "hard-reject", "OPENAI_AG", "_ensure_strict_root: root must be a non-nullable object"),
    "B4-untyped-open-object":       ("B", "hard-reject", "OPENAI_AG", "_ADDITIONAL_PROPERTIES_ERROR"),
    ...
    # anthropic.py emits `C7-enum-complex-type`; both spellings are registered
    # so lookup() can never fall through to the ("?", "unknown") default. The
    # emitted string is unchanged, so no recorded verdict moves. (0 rows in the
    # published corpus carry either spelling -- see STATS.txt.)
    "C7-enum-complex-type":              ("C", "hard-reject", "ANTH_LIMITS", "Complex types within enums"),
